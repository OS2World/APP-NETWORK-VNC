...
set components=libvncserver libgiconv vncpm os2xkey KbdXKey VNCServer VNCViewer vnckbd WPS
for %%i in (%components%) do echo Cleaning: %%i & cd %%i & cmd /c make clean & cd ..


rem Cleaning in binaries directory.

set in_bin=*.dbg *.log *.!!! vncpm.dll os2xkey.dll vncserver.exe vnckbd.sys
