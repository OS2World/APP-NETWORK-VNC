...
#ifdef _DIGI
      // We need to accept empty passwords (for ex. - for VBox VNC server)!
      if (!passwd) {
#else
...
#ifdef _DIGI
  if (major==3 && minor==8) {
    // Try to detect UltraVNC desktop name like:
    // "skh502241 ( 172.26.96.142 ) - service mode"
    char  *namepos = client->desktopName;

    while( ( *namepos != '\0' ) && ( *namepos != ' ' ) )
      namepos++;

    if ( memcmp( namepos, " ( ", 3 ) == 0 ) {
      while( ( *namepos != '\0' ) && ( *namepos != ' ' ) )
        namepos++;

      if ( strcmp( namepos, " - service mode" ) != 0 ) {
        rfbClientLog("UltraVNC server detected, enabling UltraVNC specific messages\n");
        DefaultSupportedMessagesUltraVNC(client);
      }
    }
