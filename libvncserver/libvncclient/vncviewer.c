...
#ifdef _DIGI
        if ( listenForIncomingConnectionsNoFork(client, -1) == -1 )
        {
          // Error. Avoid any subsequent operations on socket and new error
          // messages in log. The next message "read (9: Bad file number)" does
          // not make sense but last (for ex.) "ListenAtTcpPort: bind" is
          // informative.
