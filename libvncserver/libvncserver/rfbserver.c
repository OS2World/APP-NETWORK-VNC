...
#ifdef _DIGI
    // Set rfbTextChat to allow libvncclient send chat messages.
    rfbSetBit(msgs.client2server, rfbTextChat);

    // We want to inform client about file transfer support now. Else client
    // will be disconnected on first rfbFileTransfer message if file transfer
    // denied.
...
/*
   CHAR _queryDiskType(ULONG ulDrv)

   Returns type of drive ulDrv: 0 - A:, 1 - B:, e.t.c.:
     'f' - removable (floppy, flash)
     'l' - local (HDD)
     'n' - remote (network drive)
     'c' - CD/DVD
     'u' - unknown type (error)
*/
#define _UNKNOWN_DISK_TYPE         'u'
static CHAR _queryDiskType(ULONG ulDrv)
{
  ULONG			ulRC;
  #pragma pack(1)
  struct {
  ...
  // Is fixed disk (chType != 0) ?
  stParms.ucDrv = ulDrv;
  ulRC = DosDevIOCtl( (HFILE)(-1), IOCTL_DISK, DSK_BLOCKREMOVABLE, &stParms,
                      sizeof(stParms), &cbParms, &chType, 1, &cbData );
  if ( ulRC == NO_ERROR )
  {
    chType = chType == 0 ? 'f' /* removable */ : 'l' /* local */;
      ...
      else
      {
        switch ( stBPB.bDeviceType )
        {
          ...
          case 9:                 // 9, 288MB 3.5" floppy?
            chType = ulDrv <= 1 ? 'f' /* Floppy */ : 'c' /* Video disk */;
            break;
    ...
    chType = ulRC == ERROR_NOT_SUPPORTED ? 'n' : _UNKNOWN_DISK_TYPE;

  return chType;
}

/*
   rfbSendDirContent() implementation for OS/2.
*/

static VOID _ftimeOS2ToWin(RFB_FILETIME *winTime, FDATE fdate, FTIME ftime)
{
  struct tm stm;
  time_t timeFile;
  unsigned long long ticks;

  // OS/2 file date/time to unix time.
  stm.tm_year  = fdate.year + 1980 - 1900;
  stm.tm_mon   = fdate.month - 1;
  stm.tm_mday  = fdate.day;
  stm.tm_hour  = ftime.hours;
  stm.tm_min   = ftime.minutes;
  stm.tm_sec   = ftime.twosecs * 2;
  stm.tm_wday  = -1;
  stm.tm_yday  = -1;
  stm.tm_isdst = 0;
  timeFile = mktime( &stm );

  // Unix time to windows file time.
  ticks = (timeFile + 11644473600LL) * 10000000;
  winTime->dwLowDateTime = (uint32_t)ticks;
  winTime->dwHighDateTime = (uint32_t)(ticks >> 32);
}

static rfbBool _sendDirItem(rfbClientPtr cl, FILEFINDBUF3L *findBuf, BOOL fRoot)
{
  struct _ull2u32 {
    uint32_t lo;
    uint32_t hi;
  ...
  if ( findBuf->cchName == 1 && findBuf->achName[0] == '.' )
    return TRUE;

  // Don't insert directory ".." in list of root directory.
  if ( fRoot && (findBuf->attrFile & FILE_DIRECTORY) != 0 &&
       findBuf->cchName == 2 && *((PUSHORT)findBuf->achName) == 0x2E2E )
    return TRUE;

  win32filename.dwFileAttributes = findBuf->attrFile;
  _ftimeOS2ToWin(&win32filename.ftCreationTime,
                 findBuf->fdateCreation, findBuf->ftimeCreation);
  _ftimeOS2ToWin(&win32filename.ftLastAccessTime,
                 findBuf->fdateLastAccess, findBuf->ftimeLastAccess);
  _ftimeOS2ToWin(&win32filename.ftLastWriteTime,
                 findBuf->fdateLastWrite, findBuf->ftimeLastWrite);
  win32filename.nFileSizeLow = ((struct _ull2u32 *)&findBuf->cbFile)->lo;
  win32filename.nFileSizeHigh = ((struct _ull2u32 *)&findBuf->cbFile)->hi;
  win32filename.dwReserved0 = 0;
  win32filename.dwReserved1 = 0;
  strcpy((char *)win32filename.cFileName, findBuf->achName);
  win32filename.cAlternateFileName[0] = '\0';

  nOptLen = sizeof(RFB_FIND_DATA) - MAX_PATH - 14 + findBuf->cchName;

  return rfbSendFileTransferMessage(cl, rfbDirPacket, rfbADirectory, 0,
                                    nOptLen, (char *)&win32filename);
}

rfbBool rfbSendDirContent(rfbClientPtr cl, int length, char *buffer)
{
  char *basePath = alloca(length + 5);
  HDIR hdir = HDIR_CREATE;
  unsigned long rc;
  unsigned long findCnt = 1;
  ...
  if ( (length > 0) && (buffer[length-1] == '\\') )
    rc--;
  memcpy( basePath, buffer, rc );
  *((unsigned long *)&basePath[rc]) = 0x00002A5C; // Append '\\*\0\0';

  rc = DosFindFirst( basePath, &hdir,
                     FILE_READONLY | FILE_HIDDEN | FILE_SYSTEM |
                     FILE_DIRECTORY | FILE_ARCHIVED,
                     &findBuf, sizeof(findBuf), &findCnt, FIL_STANDARDL );

  if (rc != NO_ERROR)
    return rfbSendFileTransferMessage(cl, rfbDirPacket, rfbADirectory, 0, 0, NULL);

  /* send back the path name */
  if (rfbSendFileTransferMessage(cl, rfbDirPacket, rfbADirectory, 0,
                                 length, buffer) == FALSE)
  {
    DosFindClose(hdir);
    return FALSE;
  ...
  if (_sendDirItem(cl, &findBuf, *((PULONG)&basePath[1]) == 0x002A5C3A )
  //                                                        :\\\0
      != FALSE)
    while(TRUE)
    {
      findCnt = 1;
      rc = DosFindNext( hdir, &findBuf, sizeof(FILEFINDBUF3L), &findCnt );
      if (rc != NO_ERROR)
        break;
...
#else // end of __OS2__ rfbSendDirContent() implementation.
rfbBool rfbSendDirContent(rfbClientPtr cl, int length, char *buffer)
{
        ...
        case rfbRDrivesList: /* Client requests the List of Local Drives */
            ...
            */
...
#ifdef __OS2__
            {
              ...
              PCHAR      pcPos = filename2;

              if ( DosQueryCurrentDisk( &ulDiskCur, &ulDiskMap ) == NO_ERROR )
              {
                // Begin drom drive N2 (C:) - skip floppies.
                for( ulIdx = 2; ulIdx < 26; ulIdx++ )
                  if ( (( ulDiskMap << ( 31 - ulIdx ) ) >> 31) != 0 )
                  {
//                    *((PULONG)pcPos) = 0x006C3A41L + ulIdx; // 'A:l\0' + ulIdx

                    // 'A:t\0' + ulIdx , where A - drive, 't' - type (l/f/c/n).
                    *((PULONG)pcPos) = (0x00003A41L |
                                        ((ULONG)_queryDiskType( ulIdx ) << 16))
                                       + ulIdx;
                    pcPos += 4;
                  }
                *pcPos = '\0';
              ...
              retval = rfbSendFileTransferMessage(cl, rfbDirPacket,
                         rfbADrivesList, 0, pcPos - filename2, filename2);
            }
#else
    ...
    case rfbFileTransferRequest:
        ...
        */
        ...
        if (!rfbFilenameTranslate2UNIX(cl, buffer, filename1, sizeof(filename1)))
            goto fail;
#ifdef __OS2__
        cl->fileTransfer.fd=open(filename1, O_RDONLY | O_BINARY, 0744);
#else
...
#ifdef _DIGI
            /* OMG! cl->screen->getFileTransferPermission and
               cl->screen->permitFileTransfer already was checked with macro
               FILEXFER_ALLOWED_OR_CLOSE_AND_RETURN at the beginning of this
               function. I think file transfer can't be allowed for view-only
               clients. */
            return rfbSendFileTransferMessage(cl, rfbFileTransferAccess, 0,
                                              cl->viewOnly ? -1 : 1, 0 , "");
#else
            if (cl->screen->getFileTransferPermission!=NULL)
            {
