...
#ifdef __OS2__
// Hm... Application fails without this check (s != -1).
int inline __fd_isset(int s, fd_set *fds)
{ return ( ( s != -1 ) && FD_ISSET(s, fds) ); }
#undef FD_ISSET
#define FD_ISSET(__s,__fds) __fd_isset(__s,__fds)
#endif
...
#ifdef LIBVNCSERVER_WITH_WEBSOCKETS1
	    if (cl->sslctx) {
		if (rfbssl_pending(cl))
		    continue;
	    }
